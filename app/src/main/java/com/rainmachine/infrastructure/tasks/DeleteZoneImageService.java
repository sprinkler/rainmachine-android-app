...
public class DeleteZoneImageService extends GcmTaskService {
    ...
    public int onRunTask(TaskParams taskParams) {
        Timber.i("Run the gcm task to delete the zone image");
        Injector.inject(this);

        Bundle extras = taskParams.getExtras();
        String macAddress = extras.getString(EXTRA_MAC_ADDRESS);
        ...
        deleteZoneImage
                .execute(new DeleteZoneImage.RequestModel(macAddress, zoneId, new LatLong
                        (latitude, longitude)))
                .blockingSubscribe();
