...
public class BarChart extends BarLineChartBase<BarData> {
                        ...
                        if (vals == null) {
                            ...
                            for (int k = 0; k < transformed.length; k += 2) {
                                ...
                                drawValue(vals[k / 2], valuePoints[j],
                                        transformed[k + 1] + (vals[k / 2] >= 0 ? posOffset :
                                                negOffset));
                            }
                        }
                    }
    ...
    protected float getPositiveYOffset(boolean drawAboveValueBar) {
        return (mDrawValueAboveBar ? -Utils.convertDpToPixel(5) : Utils.calcTextHeight
                (mValuePaint, "8") * 1.5f);
    }

    protected float getNegativeYOffset(boolean drawAboveValueBar) {
        return (mDrawValueAboveBar ? Utils.calcTextHeight(mValuePaint,
                "8") * 1.5f : -Utils.convertDpToPixel(5));
    }
